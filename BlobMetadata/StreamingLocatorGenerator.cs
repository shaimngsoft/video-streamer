...
namespace BlobMetadata
{
    public class StreamingLocatorGenerator : IStreamingLocatorGenerator
    {
        ...
        public async Task<IDictionary<string, StreamingPath>> Generate(string blobName, Stream blob, TimeSpan? start = null, TimeSpan? end = null)
        {
            ...
            try
            {
                IAzureMediaServicesClient client = await CreateMediaServicesClientAsync(settings.MediaServices);
        ...
        private async Task<StreamingLocator> GetStreamLocator(IAzureMediaServicesClient client, string name)
        {
            ...
            IPage<StreamingLocator> locators = await client.StreamingLocators.ListAsync(settings.MediaServices.ResourceGroup, settings.MediaServices.AccountName, query);
        ...
        private async Task CleanUp(IAzureMediaServicesClient client, Asset input, Job job)
        {
            try
            {
                await client.Assets.DeleteAsync(settings.MediaServices.ResourceGroup, settings.MediaServices.AccountName, input.Name);
                if (settings.MediaServices.DeleteJobs)
                    await client.Jobs.DeleteAsync(settings.MediaServices.ResourceGroup, settings.MediaServices.AccountName, settings.MediaServices.StreamingTransformName, job.Name);
        ...
        private async Task<IDictionary<string, StreamingPath>> GetStreamingUrlsAsync(IAzureMediaServicesClient client, StreamingLocator locator)
        {
            ...
            StreamingEndpoint streamingEndpoint = await client.StreamingEndpoints.GetAsync(settings.MediaServices.ResourceGroup, settings.MediaServices.AccountName, settings.MediaServices.DefaultStreamingEndpointName);

            if (streamingEndpoint != null)
            {
                if (streamingEndpoint.ResourceState != StreamingEndpointResourceState.Running)
                {
                    await client.StreamingEndpoints.StartAsync(settings.MediaServices.ResourceGroup, settings.MediaServices.AccountName, settings.MediaServices.DefaultStreamingEndpointName);
                }
            }

            ListPathsResponse paths = await client.StreamingLocators.ListPathsAsync(settings.MediaServices.ResourceGroup, settings.MediaServices.AccountName, locator.Name);

            foreach (StreamingPath path in paths.StreamingPaths)
            {
                UriBuilder uriBuilder = new UriBuilder
                {
                    Scheme = settings.MediaServices.StreamingLocatorScheme,
        ...
        private async Task<StreamingLocator> CreateStreamingLocatorAsync(IAzureMediaServicesClient client, Asset output, string name)
        {
            ...
            locator ??= await client.StreamingLocators.CreateAsync(
                settings.MediaServices.ResourceGroup,
                settings.MediaServices.AccountName,
                name,
        ...
        private async Task<Job> SubmitJobAsync(IAzureMediaServicesClient client, string jobName, Asset input, Asset output, TimeSpan? start = null, TimeSpan? end = null)
        {
            ...
            IPage<Job> jobs = await client.Jobs.ListAsync(settings.MediaServices.ResourceGroup, settings.MediaServices.AccountName, settings.MediaServices.StreamingTransformName, query);
            ...
            job ??= await client.Jobs.CreateAsync(
                settings.MediaServices.ResourceGroup,
                settings.MediaServices.AccountName,
                settings.MediaServices.StreamingTransformName,
                jobName,
                new Job
                {
        ...
        private async Task<Transform> GetOrCreateTransformAsync(IAzureMediaServicesClient client)
        {
            ...
            Transform transform = await client.Transforms.GetAsync(settings.MediaServices.ResourceGroup, settings.MediaServices.AccountName, settings.MediaServices.StreamingTransformName);
            ...
            if (transform == null)
            {
                ...
                transform = await client.Transforms.CreateOrUpdateAsync(settings.MediaServices.ResourceGroup, settings.MediaServices.AccountName, settings.MediaServices.StreamingTransformName, output);
            }
        ...
        private async Task<Asset> CreateOutputAssetAsync(IAzureMediaServicesClient client, string assetName)
        {
            ...
            {
                StorageAccountName = settings.MediaServices.AssetStorageAccountName
            };
            return await client.Assets.CreateOrUpdateAsync(settings.MediaServices.ResourceGroup, settings.MediaServices.AccountName, assetName, parameters);
        }
        ...
        private async Task<Asset> CreateInputAssetAsync(IAzureMediaServicesClient client, string assetName, Stream blob)
        {
            ...
            Asset asset = await client.Assets.CreateOrUpdateAsync(settings.MediaServices.ResourceGroup, settings.MediaServices.AccountName, assetName, parameters);
            ...
            AssetContainerSas response = await client.Assets.ListContainerSasAsync(
                ...
                expiryTime: DateTime.UtcNow.AddHours(settings.MediaServices.AssetExpiryHours).ToUniversalTime());
        ...
        private static async Task<IAzureMediaServicesClient> CreateMediaServicesClientAsync(MediaServicesSettings config)
        {
        ...
        /// <returns></returns>
        // <GetCredentialsAsync>
        private static async Task<ServiceClientCredentials> GetCredentialsAsync(MediaServicesSettings config)
        {
